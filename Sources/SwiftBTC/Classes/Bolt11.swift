...
public enum Bolt11 {
  ...
  public static func decode(string: String) -> Invoice? {
    ...
    let date = parseTimestamp(
      data: invoiceData[invoiceData.startIndex..<invoiceData.startIndex + timestampBase32Len])
  ...
  private static func parseFallbackAddress(data: Data, network: Network) -> BitcoinAddress? {
    ...
    guard let data = data[(data.startIndex + 1)...].convertBits(fromBits: 5, toBits: 8, pad: false)
    else { return nil }
  ...
  private static func decodeAmount(for humanReadablePart: String, network: Network) -> Satoshi? {
    ...
    var amountString = humanReadablePart[
      humanReadablePart.index(
        humanReadablePart.startIndex, offsetBy: netPrefixLength)..<humanReadablePart.endIndex]
