...
extension Network {
  fileprivate var pubKeyHashAddressID: Int {
    switch self {
    case .simnet:
  ...
  fileprivate var scriptHashAddressID: Int {
    switch self {
    case .simnet:
...
public struct BitcoinAddress: Codable, Equatable, Hashable {
  ...
  public init?(witnessPubKeyHash: Data, network: Network) {
    guard
      ...
      let address = Bech32Address(
        network: network, witnessVersion: 0x00, witnessProgram: witnessPubKeyHash)?.string
    else { return nil }
  ...
  public init?(witnessScriptHash: Data, network: Network) {
    guard
      ...
      let address = Bech32Address(
        network: network, witnessVersion: 0x00, witnessProgram: witnessScriptHash)?.string
    else { return nil }
  ...
  public init(from decoder: Decoder) throws {
    ...
    guard let address = BitcoinAddress(string: string) else {
      throw DecodingError.dataCorruptedError(
        in: try decoder.singleValueContainer(), debugDescription: "Invalid Bitcoin Address")
    }
