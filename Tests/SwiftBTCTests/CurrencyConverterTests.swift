...
class CurrencyConverterTests: XCTestCase {
  func testXToSatoshi() {
    ...
    for (input, unit, satoshis) in data {
      XCTAssertEqual(
        CurrencyConverter.convert(amount: input, from: unit, to: Bitcoin.satoshi), satoshis)
    }
  }
  ...
  func testSatoshiToX() {
    ...
    for (unit, satoshis, result) in data {
      XCTAssertEqual(
        CurrencyConverter.convert(amount: satoshis, from: Bitcoin.satoshi, to: unit), result,
        "(\(unit), \(satoshis) = \(result)))")
    }
  }
}
