...
final class CurrencyTests: XCTestCase {
  private let usd = FiatCurrency(
    currencyCode: "USD", symbol: "$", localized: "US Dollar", exchangeRate: 7000.00)
  private let eur = FiatCurrency(
    currencyCode: "EUR", symbol: "€", localized: "Euro", exchangeRate: 5500.00)
  ...
  func testFiatStringToSatoshis() {
    ...
    for (currency, string, output) in data {
      XCTAssertEqual(
        currency.satoshis(from: string), output,
        "(currency code: \(currency.currencyCode), input: \(string), expected output: \(output))")
    }
  }
}
